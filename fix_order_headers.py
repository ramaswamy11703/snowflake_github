#!/usr/bin/env python3
"""
Order CSV Header Fixer

This script ensures all order CSV files have the correct header row
...
def get_order_headers():
    """
    Returns the correct header row for order CSV files
    """
    return [
...
def find_order_files(directory=".", pattern="*order*.csv"):
    """
    Find all order CSV files in the specified directory
    ...
    print(f"🔍 Found {len(files)} order CSV files:")
    for file in files:
        print(f"   - {file}")
...
def check_header(file_path):
    ...
    """
    expected_header = get_order_headers()
...
def validate_csv_structure(file_path):
    ...
    """
    expected_columns = len(get_order_headers())
...
def main():
    """
    Main function to fix headers in all order CSV files
    """
    print("🚀 Order CSV Header Fixer")
    print("=" * 50)
    ...
    order_files = find_order_files(search_directory)
    
    if not order_files:
        print("❌ No order CSV files found")
        return
    ...
    for i, file_path in enumerate(order_files, 1):
        print(f"\n📄 Processing file {i}/{len(order_files)}: {file_path}")
    ...
    # Summary
    print(f"\n📊 Summary:")
    print(f"   Total files processed: {len(order_files)}")
    print(f"   Files fixed: {fixed_count}")
    print(f"   Errors: {error_count}")
    print(f"   Already correct: {len(order_files) - fixed_count - error_count}")
